function [E_calib]=irradiation(calib, Eed, pos_soleil, T, S, ind_triangle)
%[E_calib]=irradiation(calib, Eed, pos_soleil, T, S, ind_triangle)
% fonction qui calcule l'irraidiation d'un triangle
%Paramètres d'entrée
...
for i=1:1:length(Eed)
    E_calib(i)=calib(i)*Calcul_eclairement(S1, S2, S3,pos_soleil(i,:))+Eed(i);
end
    if(FLAG_DEBUG==1)
